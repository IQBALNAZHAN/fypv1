...
//   const [mockProjects, setMockProjects] = useState(initialMockProjects);
  ...
  const saved = localStorage.getItem('mockProjects');
  return saved ? JSON.parse(saved) : initialMockProjects;
});
...
useEffect(() => {
  ...
  const getReportKey = (reportType) => {
    ...
    if (reportType.toLowerCase().includes('progress')) return 'progressReportFile';
    if (reportType.toLowerCase().includes('logbook')) return 'logbookReportFile';
    return 'reportFile';
        ...
        <div className="bg-white rounded-xl shadow-lg p-6">
            ...
            <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-indigo-50">
                    <tr>
                        <th className="px-4 py-2 text-left text-xs font-medium text-indigo-700 uppercase">Project</th>
                        <th className="px-4 py-2 text-left text-xs font-medium text-indigo-700 uppercase">Student</th>
                        <th className="px-4 py-2 text-left text-xs font-medium text-indigo-700 uppercase">Progress Report</th>
                        <th className="px-4 py-2 text-left text-xs font-medium text-indigo-700 uppercase">Logbook Report</th>
                        <th className="px-4 py-2 text-left text-xs font-medium text-indigo-700 uppercase">Supervisor Progress Report</th>
                        <th className="px-4 py-2 text-left text-xs font-medium text-indigo-700 uppercase">Supervisor Logbook Report</th>
                    </tr>
                </thead>
                <tbody className="bg-white divide-y divide-gray-200">
                    {mockProjects.map(project => (
                        <tr key={project.id}>
                            <td className="px-4 py-2 font-semibold">{project.title}</td>
                            <td className="px-4 py-2">{project.studentName}</td>
                            <td className="px-4 py-2">
                                {project.progressReportFile
                                    ? <>
                                        <span className="text-green-700">Submitted</span>
                                        <button className="ml-2 px-2 py-1 bg-blue-600 text-white rounded text-xs">Download</button>
                                      </>
                                    : <span className="text-red-500">Pending</span>}
                            </td>
                            <td className="px-4 py-2">
                                {project.logbookReportFile
                                    ? <>
                                        <span className="text-green-700">Submitted</span>
                                        <button className="ml-2 px-2 py-1 bg-blue-600 text-white rounded text-xs">Download</button>
                                      </>
                            ...
                            <td className="px-4 py-2">
                                {/* Supervisor Progress Report: auto "Submitted" if student submitted */}
                                {project.progressReportFile
                                    ? <>
                                        <span className="text-green-700">Submitted</span>
                                        <button className="ml-2 px-2 py-1 bg-blue-600 text-white rounded text-xs">Download</button>
                            ...
                            <td className="px-4 py-2">
                                {/* Supervisor Logbook Report: auto "Submitted" if student submitted */}
                                {project.logbookReportFile
                                    ? <>
                                        <span className="text-green-700">Submitted</span>
                                        <button className="ml-2 px-2 py-1 bg-blue-600 text-white rounded text-xs">Download</button>
...
const ReportSubmissionContent = ({ activeMenu, currentUser, mockProjects, setMockProjects }) => {
    const fileInputRef = useRef(null);
    const supervisorFileInputRefs = useRef({});
    const reportType = activeMenu.replace(' Submission', '');
    ...
    if (file && currentUser.role === 'Student' && studentProject) {
        setSelectedFileName(file.name);

        setMockProjects(prev =>
            prev.map(p => {
                if (p.id === studentProject.id) {
                    // Always set the correct flag for the current report type
                    let updated = { ...p, [reportKey]: file.name };
                    if (reportKey === 'progressReportFile') {
                        updated.progressReportSubmitted = true;
                    }
                    if (reportKey === 'logbookReportFile') {
                        updated.logbookReportSubmitted = true;
                    }
                    return updated;
                }
                return p;
...
// );
  ...
  return (
      ...
      <div className="flex">
        ...
        <main className="flex-1 overflow-x-hidden overflow-y-auto flex flex-col">
          ...
          <div className="flex-1">
            ...
            {activeMenu === 'Submission Overview' && <SubmissionOverviewContent />}
    ...
    <ReportSubmissionContent
        activeMenu={activeMenu}
        currentUser={currentUser}
        mockProjects={mockProjects}
        setMockProjects={setMockProjects} // <-- Add this line!
    />
)}
